...
- The consumed 0.8.10 candidate introduced the fallback with a looser version gate; review found it accepted two-part and leading-zero names and could never select a lone `0.0.0` tree. Version 0.8.11 enforces one strict gate on both platforms — exactly `X.Y.Z` with no leading zeros, while a lone `0.0.0` remains a valid first candidate — and adds adversarial regressions for `99.99`, `1`, `01.0.0`, `00.0.0`, `0.0.0.0`, `1.2.3-rc1`, and `0.0.0`. The repository validator now checks the strict-version gate and eight-event command consistency instead of only the cache-path and reinstall-hint fragments.

### Validation

- Native macOS source validation passes repository and tracked-tree privacy audits, 218 tests with five capability-aware skips, the eight-Hook self-test on Python 3.12.2, Ruff 0.16.1, external-cache compilation, commit-identity audit of the unchanged public baseline, and diff checks. Resolver regressions cover pinned-root preference, newest-survivor fallback, decoy and symlink rejection, fail-closed reinstall hints, rejection of `99.99`/`1`/`01.0.0`/`00.0.0`/`0.0.0.0`/`1.2.3-rc1`, and selection of a lone `0.0.0` tree; the PowerShell forms run on Windows CI.
- A disposable Codex CLI 0.149.0 home passes first managed install, strict repeated no-op, 15-product-file source/staging/live/archive parity, installed self-test, lifecycle smoke, and forbidden-residue checks. The macOS default home independently upgraded the consumed 0.8.10 install to exact candidate `d7c9b0b`, passed the same no-op/parity/self-test/smoke chain, reviewed and trusted all eight Hooks without bypass, and persisted schema-7 state with valid integrity after a trusted prompt and normal `SessionEnd`.
- The consumed 0.8.10 default-home evidence remains on record: the upgrade restored every host-pruned historical live tree (0.6.1–0.8.9) in one apply; a real fresh `codex exec` task fired the installed Hook and the host genuinely pruned all historical live versions again; consumed 0.8.9 bytes failed with exit 127 while installed 0.8.10 bytes returned exit 0 through the fallback and persisted schema-7 state; the next apply restored every version from the trusted archive.
- On macOS, fresh task startup and `codex plugin list` each pruned historical live caches again; the manager diagnosed the missing trees and restored all 11 indexed versions from the intact archive, then passed another strict no-op. Native Windows 10.0.26200.9168 independently passed 218 tests with six explicit capability skips, the complete PowerShell resolver gate, default-home upgrade/no-op/parity, installed self-test and lifecycle smoke, eight-Hook fresh trust, and schema-7 state readback against exact candidate `d7c9b0b`. Its fresh task did not prune historical live caches, so the observed host cleanup remains platform-specific rather than universal.
- PR #15 at exact head `d7c9b0b` passes the final 14-check matrix (Ubuntu/macOS/Windows × Python 3.10–3.13, HOL Plugin Scanner, plugin scanner). Main/tag automation, the annotated tag, GitHub Release and public readback, and downstream consumer-pin evidence remain pending. Detailed evidence is recorded in `docs/LOCAL_ACCEPTANCE.md`.
