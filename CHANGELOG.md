...
- A bounded native commentary probe connects official dynamic-tool calls, exact Hook trust readback, independent review and cold recovery to the source checks. Earlier retained macOS attempts exposed late trace publication, nested `exec` calls, default-HOME lookups and capture-publication races; focused repairs now bind the exact source and session. One later macOS run observed the answer, business continuation, real automatic compaction and cold recovery. A separate zero-model `commentary_chain/v1` profile replays its original raw evidence and passes seven scoped gates, including owned-process cleanup. Windows and the plan's control scenarios remain open.
- An explicit root wait for an exact marker now releases only when a later user message matches that marker byte for byte. A general “continue,” quoted marker, or altered case leaves it waiting; an independent future observation remains open. A bounded macOS C2 run observed one business call after the exact marker while its future observation remained pending. C1 stopped before independent review because a direct two-part question was absent from commentary source binding; the source repair needs a new installed runtime and native rerun.
...
- Source candidate under development. A reviewed macOS commentary-chain profile passes on one frozen run; its raw evidence and mapping-tool digests remain separate from the original execution commit. Focused synthetic and mutation checks do not establish Windows or the remaining control scenarios.
