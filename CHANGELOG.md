...
- Maintainers can now validate recorded Hook trust, a pause and its matching release, recovery after compaction, an observed commit, a push to a local bare repository, and cleanup. The separate `host_behavior` profile checks normally trusted raw Hook captures against exact request, state and Git readbacks. A composite validator replays each child through its original pinned adapter and validator before combining complementary gates for the same runtime and platform; it preserves separate session identities. An explicit offline replay mode can temporarily select archived captures at their original path; it requires stopped writers and restores the original directory before acceptance. Missing evidence stays `pending`, contradictions fail, and serialized summaries cannot grant a pass. The raw mapping verifies observed operation identity; it does not independently prove root-user authorization, which the runtime checks. Full results stay private; the composite produces no public annex.
