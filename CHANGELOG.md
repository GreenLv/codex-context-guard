...
- A bounded native commentary probe connects official dynamic-tool calls, exact Hook trust readback, independent review and cold recovery to the source checks. Earlier retained macOS attempts exposed late trace publication, nested `exec` calls, default-HOME lookups and capture-publication races; focused repairs now bind the exact source and session. A later macOS run on the earlier repaired installed runtime observed the complete answer, business continuation, real automatic compaction, cold recovery and the requested suite command. A separate zero-model `commentary_chain/v1` profile replays its original raw evidence and passes ten scoped gates, including captured product timepoints, suite execution and owned-process cleanup. Windows and the plan's control scenarios remain open.
- An explicit root wait for an exact marker now releases only when a later user message matches that marker byte for byte. A general “continue,” quoted marker, or altered case leaves it waiting; an independent future observation remains open. A bounded macOS C2 run observed one business call after the exact marker while its future observation remained pending. The earlier C1 binding failure remains recorded; that earlier repaired installed runtime later passed the bounded full-answer chain on macOS.
- The producer and independent reviewer share bounded process-tree ownership. Windows starts each process suspended and joins a kill-on-close Job before execution; POSIX checks group disappearance, or on macOS verifies that a remaining group has no running members. These outcomes are recorded separately. Cleanup with running or unreadable members cannot yield native acceptance, and the reviewer uses the selected isolated Host HOME. A Windows zero-model probe confirmed owned child exit after leader exit, but the installed model chain remains open.
...
- Source candidate under development. An earlier-runtime macOS full-answer run passed ten independent commentary-chain replay gates, including the actual suite command and cleanup. Its synthetic main item remains pending in final product readback, so whole-task product closure is unknown. The current runtime passed source checks, an isolated macOS install, and a Windows fixed-copy feedback comparison; it has not passed native Hook acceptance on either platform.
- The two-platform side-question contract and full release gates are still open. This candidate is not ready for release; see [development status](docs/DEVELOPMENT_0.14.2.md).
