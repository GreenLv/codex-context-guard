...
- Native Windows installation, trusted-Hook behavior, tag, GitHub Release, downstream consumer-pin evidence, and native-platform reruns beyond macOS remain pending and are not inferred from existing runs. Windows CI itself passes the full matrix at evidence head `214bfd6`, which also fixed the Windows resolver quoting after runner logs showed the host lane strips nested transport quoting. Detailed evidence is recorded in `docs/LOCAL_ACCEPTANCE.md`.
