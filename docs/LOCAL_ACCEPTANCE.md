...
The accepted runtime-tree SHA-256 is `6719501aa37601ad7f9db9a2f7672214f1c96a41cc300f6a85be2a22ba9d491d`. Version 0.13.9 is the 2026-09-14 release. Its unpublished compatible repairs retained one version; commits and runtime digests distinguish revisions, and consumed caches remain unchanged.
...
Exact commit `7e5212e1734242aec903435d06b5e209de180de3` passes Candidate
CI run `34567272445`: all twelve Ubuntu, macOS and Windows Python 3.10–3.13
lanes, the required summary, and the Windows portable installed-runtime job
pass. HOL run `34567272224` passes independently.

A separate macOS portable run on the same commit passes source identity,
isolated installation, strict second no-op, runtime parity, installed lifecycle
smoke and cleanup with no remaining IDs. Its result SHA-256 is
`8b3d24427b74ce79dc2fd42c5709a3a305298b3b35dd1fcb575d80e9e76725a2`
and its platform-local runtime-tree SHA-256 is
`e7a645457138fd1fdd3425e3432af85869f3c09d51e34556c9db8322967ee05d`;
it used Python 3.12.2 and Codex CLI 0.153.4. Independent native Windows
portable acceptance on the same commit passes the same five gates and cleanup.
Its result SHA-256 is
`be9d5111fad856170e9ecfd3802af486ebf8f86c74889ef50ce62e7cbb987aac`
and its platform-local runtime-tree SHA-256 is
`1386277389f1ff5be2e0598378b5bb571568c4c85296076fc4e2e1b39333ac81`;
it used Python 3.12.10 and Codex CLI 0.149.0. The two platform-local runtime
digests reflect their native filesystems and each matches its own installed
tree; they are not expected to be identical across platforms. The independent
Windows result is distinct from the Candidate CI Windows portable job, which
used Codex CLI 0.150.1.

The behavior fix changes runtime bytes, so the 0.13.2 portable results are not
reused. Commit `7e5212e` includes the corrected validation-tool identity and is
the exact subject of both independent portable runs. The final documentation-
only freeze does not change the plugin runtime tree; it requires its own
Candidate CI and HOL before tagging, but not another portable run. The complete
bilingual READMEs were reviewed section by section, including commands, local
and external links, status badges, tables and both rendered Mermaid diagrams.
The external registry target still advertised an older package version, so its
badge was removed; the remaining four badges load and the two diagrams render
legibly on GitHub. The reader documents pass a hash-bound bilingual cold review;
any later edit invalidates that receipt. The 0.13.2 real-host trust, continuity,
compact/resume, Git and
cleanup results remain historical evidence for 0.13.2; no fresh 0.13.3
real-host run is claimed. Tag creation, GitHub Release publication, public
readback and normal-user runtime upgrade remain separate gates.
...
| Gate | Evidence boundaries for 0.12.4 |
| --- | --- |
| Product source acceptance | P0 reproduction, P1 state/recovery and CG122-08, and P2 commit-chain source acceptance are closed at their recorded boundaries. The preceding 966-test run contains 955 passes and 11 capability-aware skips, with no failures/errors. Its original source identity is retained: the final test-only whitespace correction has an AST-equivalence/runtime-parity receipt and a fresh 20-test owning-group pass. It is not a new execution of the 966-test suite on a different source identity. |
| Validation-tools source candidate | The reviewed tooling snapshot completed a 1,035-test current-behavior suite across 33 modules: 1,024 passes, 11 skips and no failures/errors on macOS. Its 27-test composite/view group also passes from the delivered tool archive; repository/privacy audit, Ruff, compilation and diff checks pass. Earlier tools snapshots retain their separate run identities: 1,017 tests across 32 modules (1,006 passes, 11 skips), and 1,008 across 31 modules (997 passes, 11 skips), both without failures/errors. Neither historical run is a fresh execution on this successor. The recorded product runtime tree is unchanged; source checks do not establish installed full-tree parity, native Windows acceptance or CI for this successor. |
| Native macOS portable acceptance | Passed on the exact product commit and runtime above, with Python 3.12.2 and Codex CLI 0.153.4: clean source identity, isolated install, strict second no-op, runtime-tree parity, installed lifecycle smoke and cleanup. The machine-produced `native-acceptance/v2` result has SHA-256 `9362b0c663dab24426b26c923d18f38a349fa6f30082a8cf649bc783a0ff2941`. This is retained runtime evidence, not full-source parity for later tooling or documentation. |
| Native macOS real-host behavior | Passed for the same runtime and platform through a separately reviewed composite: `hook_trust`, `continuity_wait`, `compact_resume`, `commit_event`, `local_push_readback`, and `cleanup`. The composite replays two original, complementary three-gate results with their pinned adapters and validators, retains both session identities, and requires identical product subjects. It does not turn either original partial result into a six-gate run. |
| Native Windows portable acceptance | The refreshed current-tool run passes all five portable gates and isolated-home cleanup (`remaining_ids=[]`) on the product commit and runtime above, with Python 3.12.10 and Codex CLI 0.153.4. The independently read-back `native-acceptance/v2` result has SHA-256 `c6452ad1309d68548af1ae2090058e1350dba76612323df46aae82ffe3f61a27`. Tool-identity receipt `42aea8cc2054c74cd0d8ae4c71e68f3ae2ab4f299e23ef7dceb56a416599a1fb` binds the versioned entrypoint and explicit `--codex` invocation to the configured executable’s matching before/after hashes; its process/image-path observation identifies the Python runner, not each Codex child process. The earlier result `47d1b9f62df7c4eaa9359b1f19863a7b026286b37fbb042ba2d4f1e0eda9d4bc` retains its historical identity and does not replace this current-tool installation proof. Portable acceptance does not prove real-host Hook dispatch, trust or a retained logged-in home. |
| Native Windows real-host behavior | Passed through the independently reviewed R5 offline composite for the same product subject: all six gates have valid chains and exit code 0, with no failed, skipped or missing gates. The original Git/trust and continuity/cleanup sessions remain separate. Fresh replay preserves both child manifests, raw inventories and pinned tools; both temporary views and configuration were restored. Windows composite-tool tests report 26 passes and one symbolic-link privilege skip, distinct from the six native gates. R3 historical executable identity remains report-level; R4 retains its separate process and before/after evidence. The older R5 `ee8d34a5…` result retains its unrelated historical identity. |
| P3 documentation and baseline | The current local revision reconciles the new tools source result with the separately accepted macOS and Windows portable results and passes exact-byte reader review. The previous documentation review retains its original document-set identity. This closes the local evidence-text update; the separately accepted Windows composite does not close the whole P4/P5 release process. |
| Source successor acceptance | Commit `90159961678e854edf263b640b40b26e13359219` passed [Candidate CI](https://github.com/GreenLv/codex-context-guard/actions/runs/34354758784) (all 12 platform/Python lanes and required summary), [HOL](https://github.com/GreenLv/codex-context-guard/actions/runs/34354758438), Windows CI portable acceptance and macOS full-source installation parity across 117 files. A test-only UTC timestamp normalization repairs the Python 3.10 fixture; its 20-test owning module passed on Python 3.10 and 3.12. Product runtime bytes remain unchanged. |
| Final release gates | The final tag commit must independently pass Candidate CI, HOL and versioned portable acceptance. Earlier source, native and host-composite results keep their original identities and may be reused only for unchanged inputs. The release publishes the Git source/tag, with no separate package-registry artifact. Annotated tag, bilingual GitHub Release and public readback are recorded in a separate release receipt after publication; a source or installation pass alone is not that receipt. |

The Windows bootstrap result has SHA-256 `7a2541b36b308b1317cb116abdc180c6d2594ffc4e018d04f049aaa7906ca6d3`, and its capability report is `2e64cf9ef8378bb47cc59c106a50fc5592a6b5b86f7dfbbf299e0452ed8111b1`. They bind the R3 offline package to the original prepared source and explicitly leave host behavior pending. The portable entrypoint cleans its disposable home; source and carrier reuse does not imply that a persistent installed or trusted host environment exists. Earlier ownership and CRLF checkout failures retain their original failed-attempt identities; they are not portable failures for this successful run.

The explicit v2 offline archive-view mode supports separate captures that used the same canonical sink. It verifies complete raw/metadata inventories, temporarily places byte copies at that original path, and replays each child with its pinned adapter, validator, manifest and result. It preserves the starting directory and accepts the composite only after restoring it and checking its hashes. All writers must first be stopped externally: the exclusive lock coordinates replay processes, not non-cooperating recorders. A detected mutation or restoration conflict rejects acceptance and preserves unknown files and recovery paths; an interrupted process may require bounded manual recovery. The v1 mode remains read-only.

Windows R3 and R4 retain different execution-identity evidence. R3 has a historical reported executable hash; R4 records executable hashes before and after execution together with process/image-path evidence. The composite checks the product subject and platform/version compatibility; equal version text does not prove identical executable bytes, and replay does not strengthen R3's historical evidence. The Windows composite has passed independent coordinator readback within that boundary.

The accepted Windows composite result has SHA-256 `33c9037e2c44762cb8075dc767f20a8e07743a36c59205fad27cc2a59e687c37`; its v2 manifest is `8a3facea2d13b44b5ab9326af53e54692dabbf4f861c89f5e84675258be5b5ba`. Its Git/trust child remains `0ce33f676878cf04877b1d26df8214b0db509b20de5f836bd990ddedab32a999`, and its reviewed continuity/cleanup child remains `fafaae3d589dabba46aeebc7bb134b8e6a4cb556c9dc2f7413b767baf9bcfef0`. The offline run replays the preserved original captures; it does not repeat the original model, Hook lifecycle, Git operations or portable run. The 64-file Git view and 48-file continuity archive retain their original byte inventories. Independent readback confirms restored views and configuration and an absent replay lock. These receipts do not merge the original sessions or establish one shared PE identity.

The accepted macOS composite result has SHA-256 `3095008931a88efda55efd7814df6c21dd7ea3ec3b5b78dc129804148145f2ea`; its manifest has SHA-256 `f56bd2099281193d81193bf41d21cdca1d98ecb20c228c6e701be4c26ccd477c`. The Git/trust child result is `88e6c33384a112444bc75de1dbe9083a4c7137aac917c2df13f1fdd36005424d`, and the continuity/cleanup child is `012beedc928d0aba5ca6b33f830e03b9c9428e23c36d4da1740cb51a952aa2cb`. These digests identify owner-private evidence; raw captures, manifests, state snapshots, prompts and full result documents are not public repository content. The composite exports no public annex.

The real-host evidence covers the recorded Bash command-Hook shape, matching wait/release state, actual PreCompact and compact-start recovery, authorized Git operations against local bare refs, and observed cleanup that preserves protected state. It is not universal coverage of every tool wrapper, Stop wording, external wait producer, GitHub publication path or platform. Unrun variants remain separate from these passed gates. Earlier consumed 0.12.2/0.12.3 trials retain their partial/failure identities and unchanged caches.
...
`8e49604f57ebdb4d9cee6ea14208b56d3f4e27d3` freezes the prepared source,
which had SHA-256
`b899b9d887747eb8d0ae2f4e60c85808e5678a961180c65f0e6b126ffce6aaf4`
over base commit `c9c1308dc93ad2db3ebd2686f81a601616efdefe`. Its
26-file runtime tree has SHA-256
...
only successor may retain both native runtime-tree results, but it must still
