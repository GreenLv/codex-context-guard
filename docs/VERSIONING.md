...
- `0.13.9` corrects English dependency token boundaries in root pauses and aligns the native continuity draft with the supported resume syntax. Genuine external waits still require external facts; existing persisted waits are not reclassified. Schema and public protocols are unchanged. Consumed 0.13.8 caches remain immutable.
- `0.13.8` supersedes the consumed, unpublished 0.13.7 diagnostic candidate. Codex CLI 0.153.4 clamps `SessionEnd` to three seconds. The POSIX and Windows launchers now route that always-stateful event directly to the core, avoiding a second Python process; the Windows launcher also handles failed optional interpreter probes and tries a working generic Python before WindowsApps aliases. A private opt-in Windows boundary trace records phase, duration and exit code, never Hook input. The managed installer can migrate a same-repository, sanitized prior staging root named after a previous checkout in the same isolated HOME. State schema, public protocols and Hook events remain unchanged. Fresh runtime/artifact evidence is required; old caches are not rewritten.
- `0.13.7` was a consumed, unpublished diagnostic candidate. It demonstrated that raising the declared Windows `SessionEnd` timeout to ten seconds was ineffective because Codex clamped it to three; the PowerShell Python probe could fail before state persistence. Its installed cache and failed host evidence remain untouched, not acceptance for 0.13.8.
- `0.13.6` superseded the unpublished 0.13.5 source candidate. Reply delivery used a whole-request bounded grammar; unknown tails stayed pending, while supported how-to topics did not become execution obligations. State schema, public protocols and Hook events remained unchanged. Its source and native results retain their original identity.
- `0.13.5` was an unpublished answer-delivery and continuation candidate. It preserved polite acceptance criteria and added bounded resume feedback, but review found that unrecognized execution tails could still close on delivery and some how-to topics remained pending. Version 0.13.6 supersedes that classification. Its source checks remain historical, not acceptance for the successor. Historical delivery reconstruction and old caches are unchanged.
