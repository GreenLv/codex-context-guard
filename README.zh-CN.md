...
>
> 未发布的 `0.12.4` 候选修补任务续接、暂停确认、完整恢复，以及子任务结束与整体完成的区分。操作性修正措辞不再要求指出被替代项；真正的修正可以把要求写在“修正”前后，对象仍不明确时继续请求澄清。提交并推送仍绑定获授权的文件内容及已验证提交，也覆盖随后明确写出当前候选 SHA 的推送授权。已实现行为、对应回归定位，以及仍缺失的原生证据，按行为领域记录在 [0.12.4 行为基线](docs/BEHAVIOR_BASELINE_0_12_4.md)。独立验收、原生与真实宿主检查仍待完成；字节冻结前不要安装该候选。
...
- 未发布的 0.12.4 保留补充指令前的任务限制，解析要求替代关系时忽略“权限边界修正”这类操作性用法，区分不同暂停条件，按指定会话分页恢复完整要求，并把“当前候选提交”识别为已有的推送目标。schema-11 迁移、按领域的行为基线及待验收边界见[行为基线](docs/BEHAVIOR_BASELINE_0_12_4.md)和[兼容性说明](docs/COMPATIBILITY.md)。
