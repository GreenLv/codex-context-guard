...
> 源码候选：`0.14.0`（未发布、未安装到日常宿主）。一次修复完成后若只说明“长期收益仍需后续观察”，Guard 不再把这种未来观察变成现在的待办。用户若说“继续并运行测试”，当前要求和真实工具输入确定对象后，未完成测试会保留为具体要求；诚实说明当前测量尚未完成可以结束本轮，而就绪工作上的简短恢复可触发一次有界纠正。简短的“继续”不会被当作无限持续执行要求。普通编辑与测试仍由 Codex 工具执行，Guard 保存其结果供完成核验。宿主 Goal 完成拦截须先验证 Hook 路径，当前不可采用。本源码候选使用 Schema 13 和 Stop 5.0.0；共享核心 v2 契约由本仓维护，下游镜像与 pin 身份须另行核验；旧版变化见[更新日志](CHANGELOG.zh-CN.md)。

在 0.14.0 源码候选中，用户直接要求“持续完成当前任务”时，范围固定在当时已有的工作。中途插入问答不会遮蔽这些任务；后来的暂停、恢复或取消只改变有源范围。Guard 不为普通工具增加审批或执行门禁。
...
完整操作示例见 [HOL 上的 Context Guard 实践教程（英文）](https://hol.org/blog/context-guard-codex-compaction-evidence-workflow)：先定义文档要求，再压缩任务，最后核对恢复出的要求和完成的文档。
...
- 含糊输出保持 `unknown`（未知）；私有状态损坏或无法验证时，不能通过完成核验。
...
其余保持开放：本地修改、测试、普通提交、读取、搜索和 dry-run 不需要额外的 Context Guard 审批；`context-guard off` 会停止全部门禁，仅保留提示记录。普通动作被放行时屏幕上什么都不会出现；动作被拒绝时——发布合同票据失败或完整性失败——你会看到一句可操作的简短原因。
