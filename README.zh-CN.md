...
Proof protocol 1.0.0 只强制 Context Guard 可以确定性推导的义务。无法确定性验证的情况会明确退回 `legacy_fallback`（沿用较早的检查方式），不声称任意语义或像素级证明。Stop protocol 1.1.0 把完成控制限制在当前轮次；未完成处置只作为提示，不能强制开启新一轮。
...
工作过程和原生计划状态仍由 Codex 管理。Context Guard 只携带有界的正确性检查表，并在完成声明前核对证据。
