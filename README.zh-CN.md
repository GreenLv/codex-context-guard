...
> 源码候选：`0.14.0`（未发布、未安装到日常宿主）。一次修复完成后若只说明“长期收益仍需后续观察”，Guard 不再把这种未来观察变成现在的待办。用户若说“继续并运行测试”，当前要求和真实工具输入确定对象后，未完成测试会保留为具体要求；诚实说明当前测量尚未完成可以结束本轮，而就绪工作上的简短恢复可触发一次有界纠正。简短的“继续”不会被当作无限持续执行要求。普通编辑与测试仍由 Codex 工具执行，Guard 保存其结果供完成核验。宿主 Goal 完成拦截须先验证 Hook 路径，当前不可采用。本候选使用 Schema 13、Stop 5.0.0 和 provisional 共享核心 v2 身份；旧版变化见[更新日志](CHANGELOG.zh-CN.md)。
