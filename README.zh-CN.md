...
>
> 源码候选：`0.13.8`（未发布）。它修复回答交付记录、有界的继续执行提醒和 Windows `SessionEnd` 清理入口，并包含 HOL scanner 与注册表刷新更新，尚未创建 tag 或发布。自动答复关闭仅覆盖受支持的中英文信息请求，无法识别的措辞继续保留待办；详见[候选说明](docs/releases/v0.13.8.md)。
>
