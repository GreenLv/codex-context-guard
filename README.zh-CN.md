...
> 发布状态：`0.9.4` 是最近一次已发布版本；当前检出内容为未发布的 `0.9.5` 源码候选。详见[更新日志](CHANGELOG.zh-CN.md)、[兼容性说明](docs/COMPATIBILITY.md)和[本地验收记录](docs/LOCAL_ACCEPTANCE.md)。
...
- 0.9.5 源码候选会从私有检查表生成常规完成证明，Agent 不再需要为普通文件或文字结果单独写 proof JSON。它还减少了 UI 相关措辞的误判，并拒绝绑定到错误文件、任务或 Windows 路径的证据。已有状态和隐私规则不变。
...
只有用户给出明确目标时，Context Guard 才使用严格自动检查，例如指定文件或 URL、修改后必须检查的图片，或必须全部覆盖的对象清单。其他情况沿用原有完成检查，不会假装理解任意语义或图片质量。任务是否完成由通过验证的 checkpoint 决定；自然语言只用于诊断，等待或延期可以结束当前回合，但不会关闭未完成要求。

技术上的完成规则仍为 Stop protocol 2.0.0：完成措辞只用于诊断，未完成处置只是提示，不能强制开启新一轮。
