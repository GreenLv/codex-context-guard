...
> 发布状态：`0.8.4` 是最近一次已发布版本。详见[更新日志](CHANGELOG.zh-CN.md)、[兼容性说明](docs/COMPATIBILITY.md)和[本地验收记录](docs/LOCAL_ACCEPTANCE.md)。

> 源码状态：`0.8.5` 是未发布候选版，用于修正不同 commit 定址受管 checkout 之间的迁移。除非正在从源码验证该候选，请使用已发布版本。
