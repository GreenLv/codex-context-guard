...
class HostRunSupportTests(unittest.TestCase):
    ...
    def test_git_fixture_model_push_matches_mapping_contract(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory).resolve()
            RUN.fixture_git(argparse.Namespace(run_dir=root), {"runtime": {"sha256": "a" * 64}})
            folder = root / "git/fixture"
            work = folder / "work"
            remote = folder / "remote.git"
            def git(*args):
                return subprocess.run(["git", *args], cwd=work, text=True,
                                      capture_output=True, check=True)
            self.assertEqual(git("--git-dir", str(remote), "for-each-ref").stdout, "")
            git("add", "artifact.txt")
            git("-c", "user.name=Context Guard Fixture", "-c",
                "user.email=41898282+github-actions[bot]@users.noreply.github.com",
                "-c", "commit.gpgsign=false", "commit", "-m", "native-gate-acceptance")
            pushed = git("push", "origin", "HEAD:refs/heads/main")
            self.assertEqual(pushed.stderr.splitlines(),
                             [f"To {remote}", " * [new branch]      HEAD -> main"])

    def test_continuity_mapping_stage_accepts_existing_adapter_manifest(self) -> None:
        spec = importlib.util.spec_from_file_location(
            "host_continuity_fixture", ROOT / "tests/test_host_continuity_mapping.py")
        assert spec and spec.loader
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            fixture_module.MAPPING.adapt(fixture.manifest, ROOT, RUN.sha(fixture.manifest))
            item = {"runtime": {"sha256": manifest["subject"]["runtime_tree_sha256"],
                                "plugin_version": manifest["subject"]["plugin_version"]},
                    "stages": {stage: {"status": "passed"} for stage in RUN.STAGES[:4]}}
            args = argparse.Namespace(stage="mapping", proof=fixture.manifest,
                                      reviewed=True, run_dir=root)
            RUN.advance(args, item)
            self.assertEqual(item["stages"]["mapping"]["status"], "passed")
    ...
    def test_launch_environment_prioritizes_pinned_interpreters(self) -> None:
        prefix = "C:/" if RUN.os.name == "nt" else "/"
        python_dir = str(Path(prefix + "pinned"))
        shell_dir = str(Path(prefix + "shell"))
        item = {"codex_home": prefix + "isolated", "toolchain": {
            "python": {"path": str(Path(python_dir) / "python.exe")},
            "shell": {"path": str(Path(shell_dir) / "pwsh.exe")}}}
        env = RUN.launch_environment(item)
        self.assertEqual(env["CODEX_HOME"], prefix + "isolated")
        self.assertEqual(env["PATH"].split(RUN.os.pathsep)[:2],
                         [python_dir, shell_dir])
