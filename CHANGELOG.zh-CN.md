...
- 有界原生 commentary 探针将官方动态工具调用、精确 Hook 信任读回、独立审读与冷恢复接入来源核验。保留的四次 macOS 运行分别暴露答案 trace 晚到、`exec` 包装的业务子调用、观察器误读进程默认 HOME，以及审读屏障再次误读默认 HOME。第四次运行留下了一份成功的独立审读记录，但在释放业务结果前失败。离线修复现绑定嵌套身份，沿审读与冷读链显式传递隔离会话目录和宿主 HOME，并沿唯一的 `previous_response_id` 链证明先前进展确在插问前被消费。修复后源码的自动压缩和冷恢复仍需原生证据。
...
- 当前为开发中的源码候选。定向合成 Hook 和事件顺序回归不等于保留的四次原生失败记录、历史事故原链或修复后字节的完整真实模型验收。
- 可信 commentary 答案关联及完整性、双平台原生链仍未决；不宣称插问链已完整覆盖，也未达到发布条件。参见[开发状态](docs/DEVELOPMENT_0.14.2.md)。
