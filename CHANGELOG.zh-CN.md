...
- 生产者与独立审读者共用有界进程树管理。Windows 先暂停进程、加入关闭即终止成员的 Job，然后才恢复执行；POSIX 在清理后核验进程组确已消失。清理遭拒或无法证实时不能形成原生验收，审读者使用选定的隔离宿主 HOME。Windows 零模型探针证实父进程退出后仍受控的子进程在清理后退出；安装后的模型链仍待验证。
