...
- 有界原生 commentary 探针将官方动态工具调用、精确 Hook 信任读回、独立审读与冷恢复接入来源核验。当前运行时的 macOS 与 Windows 完整回答控制分别通过十项限定重放门禁。两平台的部分回答控制分别通过七项限定门禁，同时保留第二个问题和主任务待办；两次控制均未运行所请求的测试。两次部分回答运行清理前最后一次产品 SessionStart 均没有完成通知。
- 根用户明确指定精确口令的等待，现只在后续用户消息与口令逐字节相同时解除。一般“继续”、引述的口令或大小写变化均不能解除；独立的未来观察事项继续待证。macOS 与 Windows 各自的三轮控制仅在精确口令后执行一次业务调用及所请求的测试；两次零模型重放分别通过九项限定门禁，未来观察仍待办。
- 生产者与独立审读者共用有界进程树管理。Windows 先暂停进程、加入关闭即终止成员的 Job，然后才恢复执行；POSIX 核验进程组消失，或在 macOS 上核实残留进程组已无运行成员。仍有运行成员或成员状态无法读回时不能形成原生验收，审读者使用选定的隔离宿主 HOME。Windows 的三类限定控制均包含受控清理；逃逸后代进程仍未证实。
...
- 新增显式、单次独立答案审读入口与可撤销的信息义务投影。审读判断区别于宿主事实。限定范围的原生链不代表整体任务闭包，也不裁定原始 Windows 历史案例。
