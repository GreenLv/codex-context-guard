...
- 测试说明、产品描述和“权限边界修正重试”这类操作性名词短语不会取消已有要求，也不会触发替代关系澄清。要求、需求、指令、方案或计划出现在“修正”前后时仍属于权威修正；明确要求修正却没有唯一对象时仍会请求澄清。精确要求 ID 可直接确定对象，无法消除的歧义则保留原要求。未发布的 `0.12.3` 运行时字节被原生重试提示词实际使用后暴露名词短语误判；对应缓存保持不可变，其局部或失败证据不会改标为 `0.12.4`。
- 提交并推送只采用 root 用户肯定授权的文件范围；未列文件时，只采用能唯一归属当前任务的已验证修改。“提交 owned.txt，other.txt 保持不动”只允许 owned.txt；阅读、测试引用和引述指令都不增加文件。完整提交必须匹配冻结的父提交、路径、模式和 blob。
- 授权先继续修复时，文件范围保留，内容等待本次授权后的可信编辑再冻结。已有脏内容不会自动视为就绪。已经就绪的候选仍拒绝后续漂移；已暂存文件采用索引中的 blob，即使工作区内容不同。
- 配对的直接工具提交结果或唯一完整本地读回可以绑定提交，无需重复已有授权。提交失败、范围不符、基线漂移和候选歧义分别说明原因。后续 root 推送确认可引用直接工具调用已确立且仍唯一有效的目标；目标事实本身不授予权限。
- 推送重新授权会区分已有的“当前候选提交”和新建提交指令。语句中明确写出的 40 位 Git SHA 优先于工作区快照：与实时候选一致时可以继续，陈旧、来自其他仓库或以其他方式不匹配的 SHA 仍被拒绝。隔离原生试运行在发现此缺陷时已消费早期未发布的 `0.12.2` 字节；这些缓存保持不可变，其证据不会改标为 `0.12.4`。
- 缺少有界推送判定列表的早期 schema 11 状态可在恢复时保留已验证的提交观察事实。运行时先完整校验其精确四字段前序形态，再补入空判定列表；路径、摘要或记录损坏仍然 fail closed。每次直接推送只执行一次权威授权判定，并同时用于 Hook wire 与私有判定记录，避免已放行结果和记录的授权代次分叉。
- 私有状态推进到 schema 11。等待条件保存来源和对象摘要；明确适用于整个会话的限制与业务任务分别保留。schema 10 的旧等待迁移时不虚构来源或解除，schema 9 继续迁移，7/8 保持只读输入；禁止把 schema 11 有损写回 schema 10。
- 运行时子命令 `recovery-page` 必须指定 `--session-id`，按有界正文和元数据分块返回续读位置；恢复内容提供准确调用命令。默认诊断和完成检查使用同一任务适用范围。
- 唯一绑定的 `SubagentStop` 事实可以解除“等待该子任务结束”。它只证明子任务已停止，不证明子任务结果通过，也不代表父任务完成。其他外部等待仍需相应的可信事实绑定。
- 新增独立的 [0.12.4 行为基线](docs/BEHAVIOR_BASELINE_0_12_4.md)，按行为领域列出期望行为、源码与回归定位、宿主依赖、仍缺失的原生证据，以及留待 DSH 侧适配的工作。v0.9.4 对齐台账明确标记为历史快照，不再作为当前能力的描述。
- 原生验收入口在保持 portable 默认行为不变的前提下，新增独立的 `host_behavior` profile：采集器是唯一把规范化宿主记录转化为证据的组件，另有独立验证器强制因果顺序、请求/响应配对角色、事实身份绑定、重复/重放拒绝，以及会话、场景、运行时和生产者版本的一致性。新增的 owner 私有探测器记录命令 Hook 的精确 stdin 字节，自行测量已安装运行时，并校验文档规定的 Pre/PostToolUse 字段，但不产生门禁通过结论。只有在正常信任的真实运行取得原始捕获，并且原始字节到门禁事实的映射经审查接受后，整体 `passed` 才可达；此前未运行或不可验证的证据保持 `pending`，矛盾事实判为 failed，公开 annex 只保留白名单标签和摘要。
- 九个 Hook、Stop 3.0.0、分类器 3.2.1、Proof 1.0.0、Execution 2.0.0，以及 Python 3.10+ 标准库运行时保持不变。

### 验证

- 集成源码检查在记录的范围内通过。临时环境安装证据属于前一版 0.12.4 准备字节；修正后的精确候选需要重新执行安装检查，精确干净源码的 macOS/Windows 原生验收、真实宿主行为、制品、CI/HOL 与发布门禁也仍分别 pending；测试数量、身份与证据边界见[验收记录](docs/LOCAL_ACCEPTANCE.md)。候选仍未发布。
