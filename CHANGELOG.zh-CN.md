...
以下版本从新到旧排列，未发布候选会明确标注。`0.7.7` 是最新正式版本，`0.8.3` 是尚未发布的源码候选。Schema 和协议的完整历史见[版本策略](docs/VERSIONING.md)，测试过程与平台边界见[本地验收记录](docs/LOCAL_ACCEPTANCE.md)。
...
- **0.8.x——让已采用的项目指令和执行计划保持一致：** 尚未发布的 0.8 系列可以把 Skill 契约等指令来源绑定到项目执行契约，并可选择绑定当前 Codex Plan；后续发生变化时会明确标记旧绑定已经失效。

## 0.8.3 - 未发布

### 用户能感知的变化

- 长任务现在可以在原有需求和证据记录之外，保存一份经过审阅的执行清单。它会写明需要遵循哪些指令来源、分哪些阶段、通过哪些门禁，以及是否需要与当前 Codex Plan 保持一致；内部把这份清单称为“项目执行契约”。
- 这份契约默认不启用。只有发起根任务的用户审阅项目内的 JSON 契约后，显式运行 `context-guard adopt` 才会激活；普通自然语言不能自行授予执行权限。
- 已采用的计划或指令来源发生变化时，Context Guard 会把相关记录标为过期，而不会偷偷沿用旧计划。它不会改写 Codex Plan、拦截工具、自动预留任务，也不会替用户提交或发布。
...
- 两个平台上的精确提交安装和源码/缓存/归档一致性检查均已通过。Fresh-task Hook 信任、CI、tag 和 GitHub Release 仍是独立门禁，因此它还不是正式发布版本。
