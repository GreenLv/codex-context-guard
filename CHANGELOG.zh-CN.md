...
0.13.2 是未发布源码候选，macOS 源码验证通过：1,076 项 current-behavior 测试，0 失败、0 错误、11 项能力性跳过，以及仓库/隐私检查、基线 transition 审计、自检、Ruff 和编译。新的安装及原生验收待完成。0.13.1 的源码及 macOS portable 结果保留原版本身份；其原生验收发现，存在其他等待事项时，已答问题仍进入恢复待办。本候选将问题关闭与继承的等待事项分开处理。CI 和发布仍待完成。
