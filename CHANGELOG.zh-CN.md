...
- 本候选合并已安装但未发布的 0.13.0 和 0.13.1 实现，原安装缓存保持不变。升级后需启动新任务加载 0.13.2。
...
0.13.2 是未发布源码候选，macOS 源码验证通过：1,076 项 current-behavior 测试，0 失败、0 错误、11 项能力性跳过，以及仓库/隐私检查、基线 transition 审计、自检、Ruff 和编译。精确提交 `9039fa7` 的 macOS portable 隔离安装、严格二次 no-op、运行时一致性和安装态 smoke 通过；原生宿主验收仍待完成。0.13.1 的源码及 macOS portable 结果保留原版本身份；其原生验收发现，存在其他等待事项时，已答问题仍进入恢复待办。本候选将问题关闭与继承的等待事项分开处理。CI 和发布仍待完成。
