...
- 有界原生 commentary 探针将官方动态工具调用、精确 Hook 信任读回、独立审读与冷恢复接入来源核验。当前运行时的 macOS 完整回答链交付两部分答案，继续业务、真实自动压缩后运行所请求的测试，并通过十项独立重放门禁。部分回答控制保留第二个问题和主任务待办；它未运行所请求的测试，清理前最后一次 SessionStart 也没有完成通知。Windows 的完整回答链另行通过限定范围的重放，部分回答控制仍待验。
- 根用户明确指定精确口令的等待，现只在后续用户消息与口令逐字节相同时解除。一般“继续”、引述的口令或大小写变化均不能解除；独立的未来观察事项继续待证。当前 macOS 运行时的三轮控制仅在精确口令后执行一次业务调用及所请求的测试；独立零模型重放通过九项限定门禁，未来观察仍待办。Windows 尚未运行此控制。
- 生产者与独立审读者共用有界进程树管理。Windows 先暂停进程、加入关闭即终止成员的 Job，然后才恢复执行；POSIX 核验进程组消失，或在 macOS 上核实残留进程组已无运行成员。仍有运行成员或成员状态无法读回时不能形成原生验收，审读者使用选定的隔离宿主 HOME。Windows 完整回答链现包含受控清理；这不代表其余控制已验收。
...
- 独立审读现记录明确的问题与消息关联；同次投影共用来源解析，无关记录的异常载荷不再中断恢复。任务代理队列对每份新输入只认领一次。macOS 与 Windows 的有界完整回答运行均执行了审读；来源缺失或覆盖不确定时仍保持待答。
- 新增显式、单次独立答案审读入口与可撤销的信息义务投影。审读判断区别于宿主事实。限定范围的原生链不代表整体任务闭包或 Windows 其余控制已完成。
...
- 当前仍是开发中的源码候选。当前运行时的 macOS 完整回答、部分回答和精确口令运行分别通过限定范围的原生重放；Windows 完整回答链通过远端证据复核的限定重放。各原始采集器保留较窄的观察结论。完整回答运行中的合成主任务仍待办，因此整体任务的产品闭包未知。
- Windows 部分回答与精确口令控制、两条原始 Windows 历史链以及最终候选 CI/HOL 仍待验。当前尚不能发布，参见[开发状态](docs/DEVELOPMENT_0.14.2.md)和[本地验收](docs/LOCAL_ACCEPTANCE.md)。
