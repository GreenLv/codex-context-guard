...
- 有界原生 commentary 探针将官方动态工具调用、精确 Hook 信任读回、独立审读与冷恢复接入来源核验。先前保留的 macOS 运行暴露 trace 晚到、嵌套 `exec` 调用、误读默认 HOME 和捕获文件发布竞态；定向修复现绑定精确来源与会话。后续一次 macOS 运行观察到回答、业务续执行、真实自动压缩与冷恢复。独立的零模型 `commentary_chain/v1` 入口重放其原始证据，通过包括受控进程清理在内的七项限定验收门；Windows 与计划中的控制场景仍待完成。
- 根用户明确指定精确口令的等待，现只在后续用户消息与口令逐字节相同时解除。一般“继续”、引述的口令或大小写变化均不能解除；独立的未来观察事项继续待证。有界 macOS C2 运行观察到精确口令后的单次业务调用，未来观察仍待证。C1 因直接两段式问题未进入 commentary 来源绑定而在独立审读前停止；源码修复仍需新的隔离安装和原生重跑。
...
- 当前为开发中的源码候选。一条冻结 macOS 运行的 commentary 链通过经审读的限定验收入口；原执行提交与映射工具摘要分别记录。定向合成及变异测试不代表 Windows 或剩余控制场景通过。
