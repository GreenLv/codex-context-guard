...
- macOS 源码验证通过 343 项测试（9 项按能力跳过），仓库检查、公开内容审计、自检与 Ruff 均通过。聚焦测试覆盖 UI 措辞、Windows 路径、任务身份、证据读回、回滚与重复执行以及隐私边界；详细结果见[本地验收记录](docs/LOCAL_ACCEPTANCE.md)。
- 临时 Codex 目录已安装该源码候选，安装态 lifecycle smoke 通过；这没有修改日常 Codex 安装。
- 尚待完成：原生 macOS/Windows 全新 Hook 验收、候选提交的 CI、tag、GitHub Release、公开读回和任何日常安装。该候选尚未发布。
