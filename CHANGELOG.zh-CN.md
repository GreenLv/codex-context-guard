...
- “帮我讲一下这个仓库”这类仅需回复的请求，在回答交付后可以关闭；加入“运行 pytest 并总结”后，执行要求仍须保留，直到有相应证据。
- 保留礼貌措辞或问句形式的验收要求。用户明确要求继续后，若回复仍把已授权的助手工作列为下一步，最多纠偏一次；真正需要用户操作或等待外部结果时仍可结束回复。
- 纳入未发布的 0.13.4 HOL scanner 和手动注册表刷新更新。0.13.4 曾用于隔离安装，其缓存不被覆盖。

### Changes

- 将 scanner action 固定到已审查的 `v1.2.664` 不变提交，并在扫描时写出注册表 payload。只有带 `refresh_registry` 的手动触发才会请求 HOL 重新提交。
- 混合指令中无法识别的后续工作保持未完成，不因同时要求讲解而关闭。历史回答交付重建保持原样，不追溯认证已有的未完成项。
- Schema、公开协议、启用规则、默认工具访问和发布检查保持不变。运行时使用新插件版本，已有任务继续使用原有的不可变缓存。

### Validation

- 针对性的合成 Hook 测试覆盖混合请求、问句验收要求、压缩恢复、有界纠偏和真实等待。完整候选结果及限制见[本地验收记录](docs/LOCAL_ACCEPTANCE.md)。
- 先前 0.13.4 的安装和源码验证保留原有身份，不能验证本次改变后的运行时字节。真实受信任 Hook 验收、CI、HOL 摄取与发布仍是独立门禁。
