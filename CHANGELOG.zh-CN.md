...
- 维护者现在可以验证已记录的 Hook 信任、暂停及对应解除、压缩后恢复、实际观察到的提交、向本地裸仓库推送，以及清理。独立的 `host_behavior` profile 把正常信任后的原始 Hook 捕获与精确请求、状态和 Git 读回逐项核对。组合验证器先用各子结果原来锁定的适配器和验证器重放，再合并同一运行时、同一平台的互补门禁，并保留各会话的独立身份。显式离线回放模式可在原路径临时选择已归档的捕获；运行前须停止写入者，恢复原目录后才允许验收通过。缺失证据保持 `pending`，矛盾事实判为 failed，序列化摘要不能授予通过。原始映射验证观察到的操作身份，不独立证明根用户授权；授权由运行时检查。完整结果保留在私有位置；组合结果不生成公开 annex。
