...
以下版本从新到旧排列，未发布候选会明确标注。`0.10.0` 是当前源码候选，`0.9.5` 是最新正式版本。Schema 和协议的完整历史见[版本策略](docs/VERSIONING.md)，测试过程与平台边界见[本地验收记录](docs/LOCAL_ACCEPTANCE.md)。

## 0.10.0 - 未发布

### 重点

- Schema 8 让条款推导的 `(operation, subjectId, requestedSurface)` 三元组成为强制验证契约的来源：readback obligation 的主体、表面和操作均由非否定子句派生，认证会拒绝带有不同具体操作的证明证据。
- 替换 0.9.5 的词法过度生成：被否定或处于引号内的物理身份信号不再生成不可绑定 obligation，而真实请求的物理身份证明仍然失败关闭。
- Schema 7 作为只读兼容输入并确定性升级；提示词日志保持哈希化与有界，歧义 pending 操作会入账而非被丢弃，`clear-pending` 成为经过回执校验的真实控制入口且不触碰需求与证据。

### 变更

- 现有提示词、压缩、恢复、分叉、子代理、需求替代和证据重放路径继续遵循权威优先级与精确对象绑定。证据记录携带 adapter manifest 版本，证明在 manifest 漂移时失败关闭。

### 验证

- 本地源码验证已在修复后的候选字节上通过：仓库完整套件（363 通过、9 跳过）、ruff、仓库校验、公开树审计、self-test 以及干净的 `git diff --check`。CI、原生安装、工件、tag 和 release 证据仍待执行，本候选不予宣称；仓库 12 项 CI 任务保持不变。
